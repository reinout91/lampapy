{
  ...
  outputs = inputs @ {
    ...
    pyprojectOverrides = final: prev: {
      # Make the VTK wheel's .so files visible to auto-patchelf
      vtk = prev.vtk.overrideAttrs (old: {
        postFixup =
          (old.postFixup or "")
          + ''
            vtk_site="$out/lib/python3.12/site-packages/vtkmodules"
            if [ -d "$vtk_site" ]; then
              mkdir -p "$out/lib"
              for so in "$vtk_site"/*.so; do
                ln -s "$so" "$out/lib/$(basename "$so")"
              done
            fi
          '';
      });

      # Let cadquery-ocp see the PyPI vtk wheel (not nixpkgs' vtk)
      cadquery-ocp = prev.cadquery-ocp.overrideAttrs (old: {
        buildInputs = (old.buildInputs or []) ++ [final.vtk];
        # You can drop autoPatchelfIgnoreMissingDeps here and let it succeed
      });

      # Leave build123d and OCP alone unless you *know* you need to tweak them.
      # They will just use the patched cadquery-ocp + vtk.
